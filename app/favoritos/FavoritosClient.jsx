...
import { propertyPriceLabel, whatsappUrl } from "@/lib/whatsapp";

export default function FavoritosClient({ properties = [] }) {
  ...
  const favoritesSummary = favoriteProperties.slice(0, 5).map((property, index) => (
    `${index + 1}. ${property.title}\n` +
    `   ${property.location || "San Martín de los Andes"} · ${propertyPriceLabel(property)}\n` +
    `   ${SITE_URL}/propiedades/${getPropertySlug(property)}/`
  )).join("\n\n");
  const remaining = Math.max(favoriteProperties.length - 5, 0);
  const waHref = whatsappUrl(
    `Hola Milton, guardé ${favoriteProperties.length} propiedad${favoriteProperties.length !== 1 ? "es" : ""} en favoritos y quisiera compararlas con vos.\n\n` +
    favoritesSummary +
    (remaining ? `\n\nTambién guardé ${remaining} opción${remaining !== 1 ? "es" : ""} más.` : "")
  );

  const handleFavoritesWhatsApp = () => {
    trackWhatsAppClick(null, "favorites_summary");
    trackEvent("favorites_whatsapp", { property_count: favoriteProperties.length });
  };
          ...
          <>
            ...
            <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4 mb-8 p-5 rounded-2xl bg-primary-500/5 border border-primary-500/20">
              ...
              <a
                ...
                onClick={handleFavoritesWhatsApp}
