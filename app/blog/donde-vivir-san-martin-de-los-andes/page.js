...
// El dataset de barrios se comparte con las fichas /barrios/[slug].
import { PERFILES_BARRIO as BARRIOS } from "@/lib/barriosPerfil";
  ...
  return (
    <div className="border border-gray-200 rounded-2xl overflow-hidden hover:shadow-lg transition-shadow">
      ...
      <div className="px-4 py-5 sm:px-6">
        ...
        {/* Puente a la ficha del barrio: ahí están las opiniones de vecinos y
            las propiedades disponibles, que este post no muestra. */}
        <Link
          href={`/barrios/${barrio.slug}`}
          className="mt-5 inline-flex items-center gap-1.5 text-sm font-bold text-rose-600 hover:underline"
        >
          Ver la ficha completa de {barrio.nombre} →
        </Link>
      </div>
    </div>
  );
  ...
  return (
    <>
      ...
      <article className="max-w-4xl mx-auto px-4 py-8 sm:px-6 sm:py-12 lg:px-8 [hyphens:none] [overflow-wrap:normal]">
        ...
        <PodcastPlayer slug="donde-vivir-san-martin-de-los-andes" />
                ...
                {BARRIOS.map((b, i) => (
                  <tr key={b.nombre} className={`border-b border-gray-100 ${i % 2 === 0 ? "bg-white" : "bg-gray-50/50"}`}>
                    <td className="px-4 py-3 font-semibold whitespace-nowrap">
                      <Link href={`/barrios/${b.slug}`} className="text-gray-900 hover:text-rose-600 hover:underline">
                        {b.emoji} {b.nombre}
                      </Link>
                    </td>
