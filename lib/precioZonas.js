...
// Ese cruce no existía en ningún lado: el modelo publicaba la mediana POR BARRIO
// y el valor POR TIPO, pero no "los departamentos del Centro". Las 13 filas eran
...
//
// DESDE 2026-08-21 el cruce SÍ existe, y calculado: `exportar_mercado.py` publica
// `por_barrio[].por_tipo`. O sea que la pregunta que las 13 filas intentaban
// contestar ahora tiene respuesta con respaldo — y se accede con
// medianaDeBarrioPorTipo(), acá abajo. Lo que sigue prohibido es lo de antes:
  ...
  return M2_POR_BARRIO.filter((b) => b.estado === "usable" && !NO_ES_BARRIO.has(b.barrio))
    .map((b) => {
      ...
      return {
        ...
        n: b.n,
        // Desglose casa/depto del mismo barrio, `{}` si no hay comparables
        // suficientes de ningún tipo. Viaja con la fila para que quien muestre
        // la mediana mezclada pueda aclarar de qué está compuesta.
        porTipo: b.por_tipo ?? {},
      };
    })
    .sort((a, b) => b.n - a.n);
...
// Mediana del m² de un barrio PARA UN TIPO ("Casa" | "Departamento"), o null si
// el modelo no tiene comparables suficientes de ese tipo ahí.
//
// Es el número que corresponde publicar cuando se sabe qué se está vendiendo. La
// mezcla de medianaDeBarrio() promedia dos mercados distintos: en el Centro da
// 3.400, pero las casas están en 2.564 y los departamentos en 3.455. Al que tiene
// una casa en el Centro, el número mezclado le sobreestima un 33%.
//
// Devuelve null en vez de caer al número mezclado, y es deliberado: 22 de los 49
// barrios tienen desglose y varios lo tienen de un solo tipo. Un fallback
// silencioso volvería a mostrar la mezcla sin que se note — que es exactamente el
// problema. Si no hay dato, que la página decida qué hacer.
export function medianaDeBarrioPorTipo(slug, tipo) {
  const nombreModelo = SLUG_A_MODELO[slug];
  if (!nombreModelo) return null;

  const b = M2_POR_BARRIO.find((x) => x.barrio === nombreModelo);
  const dato = b?.por_tipo?.[tipo];
  if (!dato || !dato.mediana_m2_usd) return null;

  return { medianaM2: dato.mediana_m2_usd, n: dato.n, tipo, fuente: b.barrio };
}
