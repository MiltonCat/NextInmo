...
// Search Console puede devolver la misma página con variantes de host/barra y
// la misma búsqueda con o sin acentos. Las consolidamos antes de puntuar para
// no llenar el panel con recomendaciones duplicadas.
function aggregateRows(rows) {
  ...
  for (const row of rows) {
    ...
    const current = grouped.get(key) || {
      ...
      impressions: 0,
      weightedPosition: 0,
      representativeImpressions: -1,
    };
    current.clicks += clicks;
    current.impressions += impressions;
    current.weightedPosition += position * impressions;
    if (impressions > current.representativeImpressions) {
      current.keys = [page, query];
      current.representativeImpressions = impressions;
    }
...
function rowKey(row) {
  return `${pagePath(row.keys?.[0])}\n${normalizedQuery(row.keys?.[1])}`;
}
...
export function buildSeoRadar(currentRows = [], previousRows = [], rangeDays = 28) {
  currentRows = aggregateRows(currentRows);
  previousRows = aggregateRows(previousRows);
