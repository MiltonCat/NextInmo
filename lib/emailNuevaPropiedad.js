...
//
// El barrio de cada propiedad se resuelve con `barrioDePropiedad`, el mismo
// helper que usan /barrios y /precio-m2. Comparar `p.barrio` a secas dejaba
// afuera a todas las propiedades que tienen la columna vacía y el barrio
// deducido del texto de Ubicación, que en el panel es una opción legítima
// ("Deducir de la ubicación").
async function usuariosConFavoritosEnBarrio(slugBarrio) {
  const todas = await getProperties();
  const idsDelBarrio = todas
    .filter((p) => barrioDePropiedad(p)?.slug === slugBarrio)
    .map((p) => Number(p.id))
...
export async function notifyUsersAboutNewProperty(property, { soloA = null } = {}) {
  ...
  // Barrio elegido a mano, o deducido de la Ubicación si quedó sin elegir.
  const barrio = barrioDePropiedad(property)?.slug ?? null;
  ...
  if (!barrio) {
    console.warn(
      `[emailNuevaPropiedad] propiedad ${property?.id}: no se pudo determinar el barrio ` +
        `(campo vacío y la ubicación "${property?.location || ""}" no permite deducirlo). No se avisa a nadie.`
    );
