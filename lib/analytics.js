...
// Eventos de "contacto" que dispara el sitio (ver hooks/useAnalytics.js). Son
// las conversiones que de verdad importan para la inmobiliaria: alguien que da
// el paso de escribir o pedir una tasación. El orden es el que se muestra en el
// panel; se listan siempre, aunque tengan 0, para que se vea qué se está midiendo.
...
const RADAR_EVENTS = [
  "radar_view",
  "radar_signup_start",
  "radar_signup_complete",
];

const HOME_INTENT_EVENTS = [
  "home_intent_buy",
  "home_intent_sell",
  "home_intent_invest",
  "home_intent_market",
...
const TRACKED_EVENTS = [...CONVERSION_EVENTS, ...RADAR_EVENTS, ...HOME_INTENT_EVENTS];
...
export async function getAnalyticsOverview(rangeDays) {
  ...
  const [
    ...
    // Conversiones: cuántas veces se dispararon los eventos de contacto, con
    // comparativa de periodo. Se filtra a CONVERSION_EVENTS para no traer el
    // ruido de page_view, scroll y demás eventos automáticos.
    client.runReport({
      property,
      dateRanges: [current, previous],
    ...
    // Páginas que originan clics a WhatsApp. pagePath no requiere registrar
    // dimensiones personalizadas y permite detectar qué contenido convierte.
    client.runReport({
      property,
      dateRanges: [current, previous],
      dimensions: [{ name: "pagePath" }],
  ...
  // Conversiones: se arman desde CONVERSION_EVENTS para mantener orden fijo y
  // mostrar también los que tuvieron 0 (informa qué se mide aunque no haya datos).
  const convMap = new Map(
    compareByDimension(eventsResp, "eventName", "eventCount").map((e) => [e.key, e]),
  );
  const conversions = CONVERSION_EVENTS.map((name) => {
    const e = convMap.get(name) || { current: 0, previous: 0, changePct: 0 };
    return { event: name, count: e.current, prevCount: e.previous, changePct: e.changePct };
  });

  const radarFunnel = RADAR_EVENTS.map((name) => {
    const e = convMap.get(name) || { current: 0, previous: 0, changePct: 0 };
    return { event: name, count: e.current, prevCount: e.previous, changePct: e.changePct };
  });

  const homeIntents = HOME_INTENT_EVENTS.map((name) => {
    const e = convMap.get(name) || { current: 0, previous: 0, changePct: 0 };
    return { event: name, count: e.current, prevCount: e.previous, changePct: e.changePct };
  });

  const whatsappSources = compareByDimension(whatsappPagesResp, "pagePath", "eventCount")
    .map((entry) => ({
      path: entry.key,
      count: entry.current,
      prevCount: entry.previous,
  ...
  return { rangeDays, updatedAt: new Date().toISOString(), kpis, series, topPages, channels, devices, conversions, radarFunnel, homeIntents, whatsappSources };
}
...
const EVENTOS_ES = {
  ...
  generate_lead: "Consultas de propiedad",
  contact_submit: "Formulario de contacto",
  tasacion_submit: "Pedidos de tasación",
  newsletter_signup: "Suscripciones",
  radar_view: "Vieron el Radar",
  radar_signup_start: "Empezaron el formulario",
  radar_signup_complete: "Completaron la suscripción",
  home_intent_buy: "Quieren comprar",
  home_intent_sell: "Quieren vender o tasar",
  home_intent_invest: "Quieren invertir",
  home_intent_market: "Quieren conocer el mercado",
};
export function eventoES(name) {
  return EVENTOS_ES[name] || name;
}
