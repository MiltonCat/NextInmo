...
export function limpiarPregunta(texto) {
  return limpiarTexto(texto, 600);
}

// La respuesta puede ser más larga que la pregunta, pero conserva la misma
// regla de privacidad: si el modelo repite un dato personal del visitante, no
// queda guardado como parte del historial de aprendizaje.
export function limpiarRespuesta(texto) {
  return limpiarTexto(texto, 1500);
}

function limpiarTexto(texto, limite) {
...
export async function registrarPregunta(row) {
  try {
    ...
    if (!pregunta) return;
    const respuesta = row?.respondida ? limpiarRespuesta(row?.respuesta) : "";
    await rest("lucia_preguntas", {
      ...
      body: JSON.stringify({
        ...
        // La respuesta es la evidencia para revisar si Lucía contestó bien. No
        // se guarda en las guiadas ni cuando el proveedor falló.
        respuesta: respuesta || null,
        error: row.error || null,
        fuentes: Number(row.fuentes) || 0,
        herramientas: Array.isArray(row.herramientas)
          ? row.herramientas.filter((item) => typeof item === "string").slice(0, 3)
          : [],
