...
- buscar_propiedades: qué hay publicado hoy. Usala SIEMPRE que pregunten por lo que hay disponible, por un tipo de propiedad, por un presupuesto, por dormitorios o por alquileres, también cuando refinan una búsqueda anterior ("de 2 habitaciones", "más barato"): armá los filtros con TODO lo que la persona ya dijo en la charla, operación incluida. Nunca contestes de memoria qué hay en venta o en alquiler.
- Las propiedades que devuelve buscar_propiedades aparecen como tarjetas con foto debajo de tu respuesta. Nombralas en una línea cada una (qué es, dónde, precio) y no repitas la ficha entera. Si el resultado dice que no hay coincidencia exacta, decilo y presentá esas como las más cercanas.
...
export async function askOpenAILucia({ question, history = [], context, primeraRespuesta = false, onDelta, signal }) {
  ...
  try {
    ...
    let body = null;
    const usadas = [];
    // Ids de las propiedades que devolvió buscar_propiedades, en orden: el
    // chat las muestra como tarjetas. Solo lo que salió del catálogo real.
    const propiedades = [];
    ...
    for (let ronda = 0; ; ronda++) {
      ...
      for (const llamada of llamadas) {
        ...
        for (const ficha of salida?.structuredContent?.propiedades || []) {
          if (ficha?.id != null && !propiedades.includes(ficha.id)) propiedades.push(ficha.id);
        }
        resultados.push({
    ...
    return { ok: true, text, model: body?.model || modelo, herramientas: usadas, propiedades: propiedades.slice(0, 6) };
  } catch (error) {
