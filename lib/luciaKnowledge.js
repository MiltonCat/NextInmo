...
export async function buildLuciaKnowledge(question) {
  ...
  const snippets = [];
  const sources = [];
  // Aparte de `sources` porque tienen prioridad sobre las fuentes fijas: son las
  // paginas que de verdad hablan de lo que se pregunto, y al visitante se le
  // muestran solo cuatro.
  const fuentesDelIndice = [];
  const warnings = [];
  ...
  if (respuestasCasa.length) {
    ...
    for (const fila of respuestasCasa) {
      ...
      if (href) fuentesDelIndice.push(source(fila.titulo || href, href, fila.fragmento));
    }
  }
  ...
  if (delSitio.length) {
    snippets.push({
      ...
      fuente: "Parrafos textuales de paginas publicadas del sitio, ordenados por parecido con la pregunta. Se pueden citar y resumir; no completar lo que no digan.",
      items: delSitio.map((fila) => ({
        titulo: fila.titulo,
        url: fila.url,
        seccion: fila.seccion,
        texto: fila.fragmento,
      })),
    });
    // Una sola fuente por pagina aunque hayan entrado tres fragmentos de ella.
    const vistas = new Set();
    for (const fila of delSitio) {
      const href = String(fila.url || "").replace(/^https?:\/\/[^/]+/, "");
      if (!href || !href.startsWith("/") || vistas.has(href)) continue;
      vistas.add(href);
      fuentesDelIndice.push(source(fila.titulo || href, href, fila.fragmento));
    }
  }
  ...
  const asksInvesting = /inversion|invertir|rentabilidad|retorno|roi|alquiler temporario|airbnb|renta\b/.test(normalized);
  ...
  if (asksTiming || asksInvesting) {
    ...
    sources.push(source("Invertir en San Martín", "/inversiones/", "Rentabilidad por estrategia, precio del m² y comparativa de activos."));
  }
  ...
  const posts = fragmentos.length ? [] : topMatches(question, blogPosts, (post) => `${post.title} ${post.excerpt} ${post.category}`, 2);
  if (posts.length) {
    snippets.push({
      topic: "blog",
  ...
  const uniqueSources = [...new Map([...fuentesDelIndice, ...sources].map((item) => [item.href, item])).values()].slice(0, 4);
