...
const EQUIPO = {
  ...
  abogada: {
    ...
    rol: "Abogada del equipo",
    especialidad: "Revisión y redacción de contratos de locación, y control de la documentación de cada operación.",
    ...
    comoMencionarla: "Se la puede nombrar como la abogada del equipo y dar su teléfono. Es un dato ya publicado en el sitio.",
  },
  servicios: [
  ...
  limites: "Carolina asesora sobre contratos y documentación. La escritura la otorga un escribano, que es un rol distinto y lo elige la parte compradora. Ninguna respuesta de Lucía reemplaza una consulta profesional sobre un caso concreto.",
};
...
export async function buildLuciaKnowledge(question) {
  ...
  const teamQuestion = /quien(?:es)? (?:son|sos|es|esta|estan)|sobre ustedes|con quien trabaj|\bequipo\b|abogad|juridic|escriban|matricul|confiab|confianza|estafa|respaldo|trayectoria|inmobiliaria seria|milton|carolina|contrato|asesoramiento legal/.test(normalized);
  if (teamQuestion) {
    snippets.push({ topic: "equipo", facts: EQUIPO });
    sources.push(source("Nosotros", "/nosotros/", "El equipo de Catalán Propiedades"));
    if (/abogad|legal|juridic|contrato|locacion|inquilin/.test(normalized)) {
      sources.push(source(
