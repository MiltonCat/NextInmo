...
export async function buildLuciaKnowledge(question, history = [], { tasacion = null } = {}) {
  ...
  // La tasación que la persona acaba de hacer. Es el único bloque del contexto
  // que habla de UNA propiedad concreta que no está publicada, así que va
  // rotulado como lo que es y con sus reglas encima: sin eso el modelo lee un
  // valor suelto y lo repite como un precio de la casa. Llega validado desde
  // /api/lucia (lib/luciaTasacion.mjs); acá ya se confía en la forma.
  if (tasacion) {
    snippets.push({
      topic: "tasacion_del_visitante",
      fuente: "Es la tasación que esta persona acaba de hacer en el tasador del sitio, recién, en esta misma visita. Es la estimación de nuestro modelo predictivo para SU propiedad: no es una propiedad publicada, ni una tasación cerrada, ni un precio que la casa haya confirmado.",
      reglas: [
        "Contestá sobre esta propiedad y estos números. No los uses para otra consulta ni los mezcles con propiedades del catálogo.",
        "El valor nunca se dice como certeza: va siempre con el rango, y el error promedio del modelo tiene que aparecer en la respuesta, dicho como lo diría una persona.",
        "Los valores del barrio son precios publicados, no de cierre. El desvío está calculado contra `medianaComparadaM2USD` y `baseDeComparacion` dice de qué está hecha esa mediana: cuando es el barrio entero —casas y departamentos mezclados— decilo, porque son dos mercados distintos y parte de la diferencia puede ser de la comparación y no de la propiedad.",
        "El modelo recibe las características y extras declarados, incluida la vista o a estrenar cuando se seleccionaron. No inspecciona la propiedad ni verifica la calidad de esa vista, el estado real, las terminaciones, la orientación, el ruido ni los papeles. Milton evalúa esos aspectos en la revisión. No digas que el modelo ignora un extra que sí recibió.",
        "No digas si está cara o barata, ni recomiendes a cuánto publicarla: mostrá qué dice el dato, qué puede explicar la diferencia contra el barrio y ofrecé la revisión con Milton.",
        "No recibís el aporte individual de cada característica al cálculo: no atribuyas montos, porcentajes, ni un orden de importancia a barrio, metros o extras. Podés explicar las entradas declaradas, el rango y la comparación; no inventes una explicación causal del valor.",
        "Ningún dato de la propiedad que no esté acá. Si falta algo que hace falta para responder, pedíselo en una línea.",
      ],
      datos: tasacion,
      fechaDelContextoDeMercado: MERCADO_GENERADO,
      alcance: "El resultado es una estimación del tasador; objetivo y plazo son declaraciones del visitante. Las medianas son referencias agregadas, no comparables individuales verificados.",
    });
    addSources(source("Tasador online", "/tasacion/", "La estimación que acaba de hacer esta persona."));
  }
  ...
  // La tasación del visitante va arriba de todo lo demás salvo la ficha de la
  // casa: si el contexto se recorta por tamaño, lo último que puede faltar es
  // justo el número por el que la persona está preguntando.
  const prioridad = ["empresa", "tasacion_del_visitante", "respuesta_de_la_casa", "tasador_publicado", "inversion_publicada", "mercado", "contenido_del_sitio", "blog"];
