...
// Un desarrollo con `oculto: true` no existe para el sitio: no sale en el
// listado, no tiene ficha, no entra al sitemap y no se pre-renderiza. El filtro
// vive acá y no en cada página justamente para eso — si cada pantalla se
// acordara por su cuenta de mirar la bandera, alcanza con que una se olvide
// para que el desarrollo que Milton quiso esconder siga teniendo URL propia.
const visibles = (lista) => lista.filter((d) => !d.oculto);

export async function getDevelopments() {
  const client = getClient();
  if (!client) return visibles(fallbackDevelopments);
  ...
  if (error || !data || data.length === 0) {
    ...
    return visibles(fallbackDevelopments);
  }
  return visibles(data.map(normalize));
}
...
// Acá vivía estaDisponible(), que daba false para los agotados además de para
// los ocultos. Nunca la llamó nadie, y menos mal: el listado muestra los
// agotados A PROPÓSITO — una obra vendida entera es la mejor prueba de que el
// desarrollador entrega. Lo único que saca a un desarrollo de circulación es
// `oculto`, y de eso se encarga visibles() acá arriba.
