...
// Para nombrar la mediana en la frase que lee el modelo. Son los dos únicos
// tipos del tasador y los dos únicos con desglose en el JSON del modelo.
...
export const OBJETIVOS_TASACION = ["Solo conocer el valor", "Estoy pensando en vender", "Quiero vender", "Estoy evaluando una compra"];
export const PLAZOS_TASACION = ["Sin plazo definido", "En los próximos 3 meses", "Entre 3 y 6 meses", "Más adelante"];

// Los dos puntos de entrada al chat usan el mismo contrato sin normalizarlo
// todavía: la API valida el resultado al recibir cada consulta.
export function crearTasacionParaLucia({ resultado, contexto, datos }) {
  return {
    tipo: datos.tipo,
    barrio: datos.barrio,
    ...
    valorM2: resultado.valorM2,
    rangoMin: resultado.rangoMin,
    rangoMax: resultado.rangoMax,
    errorPromedioPct: resultado.errorPromedioPct,
    nEntrenamiento: resultado.nEntrenamiento,
    advertencias: resultado.advertencias,
    medianaBarrio: contexto?.medianaBarrio,
    nBarrio: contexto?.nBarrio,
    medianaBarrioTipo: contexto?.medianaBarrioTipo,
    nBarrioTipo: contexto?.nBarrioTipo,
...
export function normalizarTasacion(entrada) {
  ...
  const medianaBarrioTipo = numero(entrada.medianaBarrioTipo, { min: 1, max: TOPE_M2_USD });
  const nBarrioTipo = entero(entrada.nBarrioTipo, { min: 1, max: 100_000 });
  const terreno = numero(entrada.superficieTerreno, LIMITES.terreno);

  // Contra qué se compara el m². La mediana del barrio mezcla casas y
  // departamentos; la del tipo es la que corresponde cuando sabemos qué se
  // tasó, y es la que la persona está viendo en pantalla. El desvío se calcula
  // contra ESA, y el objeto dice de cuál se trata: si Lucía no supiera de qué
  // está hecha la mediana, explicaría una diferencia que no es la que hay.
  const medianaComparada = medianaBarrioTipo ?? medianaBarrio;
  const baseDeComparacion = medianaComparada === null
    ? null
    : medianaBarrioTipo !== null
      ? `precios publicados de ${PLURAL_TIPO[tipo] || "propiedades"} de ${barrio}`
      : `precios publicados de ${barrio}, casas y departamentos juntos (no hay suficientes ${PLURAL_TIPO[tipo] || "propiedades"} relevadas ahí para una mediana propia)`;
  ...
  return {
    tipo,
    barrio,
    objetivo: OBJETIVOS_TASACION.includes(entrada.objetivo) ? entrada.objetivo : null,
    plazo: PLAZOS_TASACION.includes(entrada.plazo) ? entrada.plazo : null,
    ...
    cocheras: entrada.cocheras == null ? null : entero(entrada.cocheras, LIMITES.cocheras),
    ...
    medianaBarrioTipoM2USD: medianaBarrioTipo === null ? null : Math.round(medianaBarrioTipo),
    nBarrioTipoRelevadas: nBarrioTipo,
    medianaComparadaM2USD: medianaComparada === null ? null : Math.round(medianaComparada),
    baseDeComparacion,
    ...
    desvioVsMedianaPct: valorM2 !== null && medianaComparada !== null
      ? Number((((valorM2 - medianaComparada) / medianaComparada) * 100).toFixed(1))
      : null,
    advertenciasDelModelo: advertencias,
  };
}

export function resumenTasacionParaMilton(entrada, mensajes = []) {
  const t = normalizarTasacion(entrada);
  if (!t) return null;
  const usd = (n) => `USD ${n.toLocaleString("es-AR")}`;
  return [
    "Hola Milton, quisiera revisar la tasación que hice con Lucía.",
    `${t.tipo} en ${t.barrio}${t.superficieCubiertaM2 ? `, ${t.superficieCubiertaM2} m² cubiertos` : ""}.`,
    t.objetivo && `Mi objetivo: ${t.objetivo}.`,
    t.plazo && `Plazo: ${t.plazo}.`,
    `Estimación orientativa: ${usd(t.valorEstimadoUSD)}.`,
    t.rangoUSD && `Rango: ${usd(t.rangoUSD.min)} a ${usd(t.rangoUSD.max)}. No es un precio de cierre.`,
    ...t.advertenciasDelModelo,
    ...mensajes.filter((m) => m.role === "user").slice(-3)
      .map((m) => `Lo que comenté: ${texto(m.text, 350)}`),
  ].filter(Boolean).join("\n");
}
