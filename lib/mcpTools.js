...
import { dormitoriosDe, eligibleProperties } from "@/lib/luciaAdvisor.mjs";
...
function fichaDePropiedad(p, alquiler) {
  return {
    // El id viaja para que el chat muestre la tarjeta de la propiedad debajo
    // de la respuesta; el modelo no lo necesita para redactar.
    id: p.id,
    titulo: p.title,
...
// Lo que el modelo manda como tipo no siempre es un tipo: "alquiler",
// "propiedad", "cualquiera" o "Departamento o PH" recortaban el catálogo a cero
// y Lucía contestaba "no hay alquileres" con cinco publicados. Solo se filtra
// por las palabras que de verdad aparecen en el campo `type`.
const TIPOS_CONOCIDOS = ["casa", "departamento", "ph", "monoambiente", "cabana", "lote", "terreno", "local", "duplex"];

export function tiposDeLaBusqueda(tipo) {
  const texto = normalizar(tipo);
  if (!texto) return undefined;
  const tipos = TIPOS_CONOCIDOS.filter((clave) => new RegExp(`\\b${clave}`).test(texto));
  return tipos.length ? tipos : undefined;
}

async function buscarPropiedades(args = {}) {
  // "alquiler", "alquilar", "alquiler permanente": el enum del esquema no se
  // impone del lado de OpenAI, así que cualquier variante cuenta.
  const alquiler = /alquil/.test(normalizar(args.operacion));
  ...
  const filtros = {
    ...
    types: tiposDeLaBusqueda(args.tipo),
  ...
  // Sin coincidencia exacta, pero con catálogo de esa operación: se muestra lo
  // más cercano y se dice que no es exacto. "No hay nada" solo cuando de verdad
  // no hay nada publicado de esa operación.
  let aproximado = false;
  if (!encontradas.length && (filtros.types || filtros.minBedrooms || filtros.maxPrice)) {
    const base = eligibleProperties({ operacion: filtros.operacion }, catalogo.filas);
    if (base.length) {
      encontradas = base;
      aproximado = true;
    }
  ...
  return {
    content: texto([
      aproximado ? "No hay coincidencia exacta con esos filtros. Estas son las disponibles más cercanas:" : null,
    ...
    structuredContent: { total, aproximado, propiedades: items, listado },
  };
}
...
async function perfilBarrio(args = {}) {
  ...
  const ficha = {
    ...
    // "¿A qué distancia está?" es la pregunta del que no conoce la ciudad. Si
    // todavía no está cargada, el campo no viaja: nada de aproximados.
    distancias: textoDistancias(perfil.slug),
    puntajes_sobre_5: {
      tranquilidad: perfil.tranquilidad,
      servicios: perfil.servicios,
      acceso: perfil.acceso,
      seguridad: perfil.seguridad?.nivel ?? null,
    },
    calles: perfil.calles?.estado ?? null,
    cloacas: perfil.cloacas?.estado ?? null,
    seguridad: perfil.seguridad?.detalle ?? null,
    salud: (perfil.hospital?.items || []).slice(0, 3).map((i) => `${i.nombre} (${i.distancia})`),
    pet_friendly: perfil.petFriendly ?? null,
  ...
  return {
    content: texto([
      ...
      ficha.calles ? `Calles: ${ficha.calles}.` : null,
      ficha.cloacas ? `Cloacas: ${ficha.cloacas}.` : null,
      ficha.seguridad ? `Seguridad: ${ficha.seguridad}.` : null,
      ficha.internet ? `Internet: ${ficha.internet}` : null,
      ficha.transporte ? `Transporte: ${ficha.transporte}` : null,
      ficha.salud.length ? `Salud cerca: ${ficha.salud.join(" · ")}` : null,
...
// Las `annotations` no son decorativas: OpenAI las revisa a mano al aprobar la
// app en el directorio de ChatGPT, y tienen que describir lo que la herramienta
// hace de verdad. Las tres son iguales porque las tres solo leen datos ya
// publicados del propio catálogo: nada que escriba, borre ni salga a internet
// abierta. Si alguna vez se agrega una que reserve una visita o mande un mail,
// esa NO lleva readOnlyHint.
const SOLO_LECTURA = {
  readOnlyHint: true,
...
// ------------------------------------------------- las mismas, para el chat
//
// Lucía y el servidor MCP ofrecen exactamente las mismas herramientas, y se
// derivan de HERRAMIENTAS para que no haya dos listas que se desincronicen. Lo
// único que cambia es la forma: MCP usa `inputSchema`, la API de OpenAI usa
// `parameters`.
export function herramientasParaOpenAI() {
  return HERRAMIENTAS.map((herramienta) => ({
    type: "function",
    name: herramienta.name,
    description: herramienta.description,
    parameters: { ...herramienta.inputSchema, additionalProperties: false },
  }));
}

// La salida MCP es una lista de bloques; el modelo espera un texto plano.
export function textoDeSalida(salida) {
