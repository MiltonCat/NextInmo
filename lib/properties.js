...
export async function getProperties() {
  ...
  if (error || !data || data.length === 0) {
    // Una tabla vacía se trata igual que un error: el visitante ve el respaldo
    // en los dos casos, y el catálogo real no llegó ni en uno ni en otro.
    marcarDegradado("properties", error ? error.message : "la consulta no devolvió filas");
    return fallbackProperties.map(normalize);
  }
