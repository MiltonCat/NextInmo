...
// Supabase es la fuente de verdad. Ante una falla, no sustituir sus datos por
// el catálogo estático: puede tener precios y disponibilidades anteriores.
import "server-only";
...
function normalize(row) {
  ...
  // migración—, se arma desde los cinco campos
...
export async function getProperties() {
  ...
  if (!client) {
    // Faltan las credenciales. En desarrollo es un olvido; en producción es
    // exactamente la falla silenciosa que buscamos, así que se anuncia igual.
    marcarDegradado("properties", "sin credenciales de Supabase");
    throw new Error("No se pudo consultar el catálogo. Intentá nuevamente.");
  }
  ...
  try {
    ...
    // Cero filas es un catálogo vacío válido, no una caída de la base.
    return data.map(normalize);
  } catch (error) {
    marcarDegradado("properties", error?.message || "falló la consulta");
    // Next conserva la última página ISR válida si falla su revalidación.
    // Sin caché, el error boundary permite reintentar; nunca inventamos datos.
    throw new Error("No se pudo consultar el catálogo. Intentá nuevamente.");
  }
}
