...
export function normalizarEntrada(body = {}, ciudad = "sma") {
...
export async function getBarrios(ciudad = "sma") {
  ...
  try {
    const res = await fetch(`${TASADOR_API_URL}/barrios?ciudad=${encodeURIComponent(ciudad)}`, {
      // La lista depende de la configuración activa de la API. No cachear
      // respuestas vacías de una ciudad recién habilitada.
      cache: "no-store",
...
export async function despertarModelo(ciudad = "sma") {
  const controlador = new AbortController();
